require "application_system_test_case"

class BookingsTest < ApplicationSystemTestCase
  setup do
    @booking = bookings(:one)
  end

  test "visiting the index" do
    visit bookings_url
    assert_selector "h1", text: "Bookings"
  end

  test "should create booking" do
    visit bookings_url
    click_on "New booking"

    fill_in "Date of service", with: @booking.date_of_service
    fill_in "Expected fee", with: @booking.expected_fee
    fill_in "First name", with: @booking.first_name
    fill_in "Last name", with: @booking.last_name
    fill_in "Pet name", with: @booking.pet_name
    fill_in "Pet type", with: @booking.pet_type
    fill_in "Time span", with: @booking.time_span
    click_on "Create Booking"

    assert_text "Booking was successfully created"
    click_on "Back"
  end

  test "should update Booking" do
    visit booking_url(@booking)
    click_on "Edit this booking", match: :first

    fill_in "Date of service", with: @booking.date_of_service
    fill_in "Expected fee", with: @booking.expected_fee
    fill_in "First name", with: @booking.first_name
    fill_in "Last name", with: @booking.last_name
    fill_in "Pet name", with: @booking.pet_name
    fill_in "Pet type", with: @booking.pet_type
    fill_in "Time span", with: @booking.time_span
    click_on "Update Booking"

    assert_text "Booking was successfully updated"
    click_on "Back"
  end

  test "should destroy Booking" do
    visit booking_url(@booking)
    click_on "Destroy this booking", match: :first

    assert_text "Booking was successfully destroyed"
  end
end
